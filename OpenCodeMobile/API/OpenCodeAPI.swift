...
struct OpenCodeAPI {
    ...
    func session(id: String, directory: String? = nil) async throws -> Session {
        try await client.get(Session.self, "/session/\(id)", directory: directory)
    }

    func deleteSession(id: String, directory: String? = nil) async throws -> Bool {
        try await client.delete(Bool.self, "/session/\(id)", directory: directory)
    }

    func patchSession(id: String, title: String, directory: String? = nil) async throws -> Session {
        let data = try await client.request("PATCH", "/session/\(id)", directory: directory, body: ["title": title])
    ...
    func abortSession(id: String, directory: String? = nil) async throws -> Bool {
        try await client.post(Bool.self, "/session/\(id)/abort", directory: directory)
    }
    ...
    func replyPermission(sessionID: String, permissionID: String, response: PermissionResponse, directory: String? = nil) async throws -> Bool {
