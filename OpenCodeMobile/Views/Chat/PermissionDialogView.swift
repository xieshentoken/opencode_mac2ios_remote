...
struct PermissionDialogView: View {
    ...
    private var commandText: String? {
        if case .string(let c)? = permission.metadata?["command"] { return c }
        if case .string(let s)? = permission.metadata?["pattern"] { return s }
        if case .string(let s)? = permission.metadata?["description"] { return s }
        return permission.patterns?.first
    }
}
